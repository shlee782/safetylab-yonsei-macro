...
2. main.py를 실행시킵니다. 순서대로 'day: '와 'check_type: '이 출력될텐데, 전자에서는 점검 일자를 입력하면 되고 후자에서는 점검을 해야할 날짜인 경우 'a'르 공휴일 등의 이유로 점검이 필요하지 않으 날짜인 경우 'b'라고 입력하면 됩닏.
